...
namespace WebBanMyPham.Areas.Administrator.Controllers
{
    public class CommentController : BaseController
    {
        ...
        {
            //check logged
            var listMenu = new List<CategoryProduct>();
        ...
        public JsonResult ListMenuByAjax(string languageId)
        {
            ...
            using (var db = new MyDBDataContext())
            {
                ...
                var listLanguageTemp = db.CategoryProducts.Select(a => new Options() { DisplayText = a.Title, Value = a.ID.ToString(CultureInfo.InvariantCulture) }).ToList();
